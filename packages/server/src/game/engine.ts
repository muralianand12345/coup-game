...
export const getAlivePlayersCount = (state: GameState): number =>
...
export const playerHasCard = (player: Player, cardType: CardType): boolean =>
  player.cards.some(c => c.type === cardType && !c.isRevealed);

export const getPlayerUnrevealedCards = (player: Player): Card[] =>
  player.cards.filter(c => !c.isRevealed);
...
export const getPublicGameState = (state: GameState, forPlayerId: string): GameState => {
  return {
    ...
    players: state.players.map(p => ({
      ...p,
      cards: p.cards.map(c => 
        p.id === forPlayerId || c.isRevealed
          ? c
          : { ...c, type: CardType.DUKE, id: c.id }
      ),
    })),
