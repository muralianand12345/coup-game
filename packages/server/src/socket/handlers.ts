  ...
  io.on('connection', (socket: TypedSocket) => {
    ...
    socket.on('challenge', () => {
      ...
      if (state.phase === GamePhase.ACTION_RESPONSE && state.pendingAction?.canBeChallenged) {
        const challengeSuccess = GameEngine.handleChallenge(
          state,
          playerId,
          state.pendingAction.requiredCard!,
        ...
        const challengeSuccess = GameEngine.handleChallenge(
          state,
          playerId,
          state.pendingBlock.claimedCard,
          state.pendingBlock.playerId
        );
    ...
    socket.on('block', (cardType: CardType) => {
      ...
      if (action.targetRequired && action.targetId !== playerId) {
        if (action.type !== ActionType.FOREIGN_AID) return;
      }
